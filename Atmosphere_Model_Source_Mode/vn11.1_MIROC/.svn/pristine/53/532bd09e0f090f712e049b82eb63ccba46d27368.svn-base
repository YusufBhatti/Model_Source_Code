...
!$OMP CRITICAL (asad_bimol_init)
 ...
 !                                 jpbk+1, jpspb )
   ! ELSE IF (l_ukca_strat .OR. l_ukca_strattrop) THEN
      !   DMS + OH => SO2 + MSA
!      prods(2) = 'MSA       '
!      idmsoh = asad_findreaction( r1, r2, prods, 2, spb, nbrkx,           &
!                                  jpbk+1, jpspb )
...
IF ( idmsoh /= 0) THEN
  ...
  tmp(1:n_points) = 350.0*inv_t(1:n_points)
  CALL exp_v(n_points,tmp,tmp_out)
  beta(:) = 3.04e-12*tmp_out(1:n_points)
  ...
  rk(1:n_points,idmsoh) = rk(1:n_points,idmsoh)*beta *                  &
                          alpha(:)/(1.0 + alpha(:))
!  tmp(1:n_points) = 7460.0*inv_t(1:n_points)
!  CALL exp_v(n_points,tmp,tmp_out)
!  alpha(:) = 1.106e-31*tmp_out(1:n_points)* tnd(1:n_points)
