...
REAL, ALLOCATABLE :: beta(:)              ! Multiplication factor YAB
...
IF ( idmsoh /= 0) THEN
  ALLOCATE(alpha(1:n_points))
  ALLOCATE(beta(1:n_points))
  tmp(1:n_points) = 5376.0*inv_t(1:n_points)
  CALL exp_v(n_points,tmp,tmp_out)
  alpha(:) = 8.20e-39*tmp_out(1:n_points) * f(1:n_points,io2)

  tmp(1:n_points) = 3644.0*inv_t(1:n_points)
  CALL exp_v(n_points,tmp,tmp_out)
  beta(:) =1.05e-5*tmp_out(1:n_points)* tnd(1:n_points)

  rk(1:n_points,idmsoh) = rk(1:n_points,idmsoh)*                  &
                          alpha(:)/(1.0 + beta(:) * 0.2095)
ENDIF
...
IF (ALLOCATED(beta)) DEALLOCATE(beta) ! YAB add beta 
